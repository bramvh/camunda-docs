...
When super-user mode is activated, the users are temporarily granted **Project Admin** access to all projects
...
The user must be assigned as organization **Owner** or **Admin**. This can be done in Console.

</TabItem>

<TabItem value='self-managed'>

The user must be assigned the **Web Modeler Admin** role in Identity.

If the role is not pre-existing, it should be created with the following permission:
